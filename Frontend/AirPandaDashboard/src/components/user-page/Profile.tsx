...
const Profile = () => {
  ...
  const { userSelected, setUserSelected, image, setImage } =
  ...
  const fetchUser = async () => {
    setLoading(true);
    try {
      const response = await getUserById(userSelected[0]);
  ...
  useEffect(() => {
    const validateForm = () => {
      ...
      if (!firstName || !lastName || !email || !role || openToEdit) {
        return true;
      }
  ...
  }, [firstName, lastName, email, role, password, confirmPassword, openToEdit]);
  ...
  useEffect(() => {
    if (user) {
      const userUpdate: string[] = [user.id, user.userName];
      setUserSelected(userUpdate);
    }
  }, [user]);
  ...
  return (
    <>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="transition-border duration-500 ease-in-out w-full p-3 rounded-2xl border border-gray-400 bg-white text-black dark:text-white/60 dark:bg-[#181818] dark:border-white/60 disabled:border-none dark:disabled:border-none disabled:bg-gray-100 dark:disabled:bg-white/10 disabled:text-gray-400 dark:disabled:text-white/30 focus:outline-none focus:bg-blue-50 appearance-none"
                    >
      ...
      <ImgModal isOpen={isOpen} setIsOpen={setIsOpen} />
    </>
  );
};
